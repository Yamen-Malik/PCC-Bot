...
class Welcome(app_commands.Group):
    ...
    status_group = app_commands.Group(
        name="status", description="Get or change the status of welcome new members"
    )
    channel_group = app_commands.Group(
        name="channel", description="Get or change the welcome test channel"
    )
    messages_group = app_commands.Group(
        name="messages", description="Configure welcome messages"
    )
    ...
    async def get_welcome_status(self, interaction: Interaction) -> None:
        ...
        await interaction.response.send_message(
            f"Welcome new members: {welcome_new_members}"
        )
    ...
    async def set_welcome_status(self, interaction: Interaction, active: bool) -> None:
        ...
        await interaction.response.send_message(
            f"Welcome new members has been updated to: {active}"
        )
    ...
    async def set_welcome_channel(
        ...
        await interaction.response.send_message(
            f"welcome channel has been updated to: {channel.name}"
        )

    @messages_group.command(name="get")
    async def get_welcome_messages(self, interaction: Interaction) -> None:
        ...
        embed = Embed(
            title="welcome messages", description=messages_str, color=Color.green()
        )
        await interaction.response.send_message(embed=embed)

    @messages_group.command(name="add")
    async def add_welcome_messages(
    ...
    @messages_group.command(name="delete")
    async def delete_welcome_messages(
        self, interaction: Interaction, message_index: int
    ) -> None:
        ...
        # validate index
        if message_index < 0 or message_index >= len(
            db[str(guild_id)]["welcome_messages"]
        ):
            await interaction.response.send_message(
                "Invalid message index", ephemeral=True
            )
            return

        # delete message
        deleted_message = db[str(guild_id)]["welcome_messages"].pop(message_index)

        await interaction.response.send_message(
            f"Message have been deleted: {deleted_message}"
        )
