...
class Poll(app_commands.Group):
    ...
    async def validate_options(self, interaction: Interaction, options: list[str]) -> bool:
    ...
    async def poll_reactions(self, interaction: Interaction, poll_name: str, options: str, emojis: str=DEFAULT_OPTION_EMOJIS,
        ...
        if len(options) != len(emojis):
            ...
            return
        elif not (await self.validate_options(interaction, options) and await self.validate_options(interaction, emojis)):
            return
    ...
    async def result(self, interaction: Interaction, poll_name: str) -> None:
        ...
        if not poll["public_results"] and poll["author_id"] != interaction.user.id and not interaction.user.guild_permissions.manage_messages:
            await interaction.response.send_message("You don't have permission to view the results of this poll.", ephemeral=True)
            return
    ...
    async def close(self, interaction: Interaction, poll_name: str) -> None:
        ...
        # send poll result
        await self.result.callback(interaction, poll_name)
