...
async def send_rand(ctx: commands.Context, l, u, is_int: bool) -> bool:
    """
...
@command
async def rand(ctx: commands.Context, l: int = 0, u: int = 1) -> bool:
    return await send_rand(ctx, l, u, False)


@commands.command(name="rand_int", help="Generate a random integer in the given range [l, u]")
@command
async def rand_int(ctx: commands.Context, l: int = 0, u: int = 100) -> bool:
    return await send_rand(ctx, l, u, True)


@commands.command(name="rand_choice", help="Returns a random value from the given values")
@command
async def rand_choice(ctx: commands.Context, *args: tuple[str]) -> bool:
    if not args:
        return False
...
@command
async def rand_user(ctx: commands.Context, server_wide: bool = False) -> bool:
