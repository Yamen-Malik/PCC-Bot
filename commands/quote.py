...
class Quote(app_commands.Group):
    ...
    async def random(self, interaction: Interaction, tag: str = None) -> None:
        ...
        quote_response = requests.get(self.quote_api_url + endpoint)
        ...
        author_link = None
        try:
            authors_response = requests.get(f"{self.quote_api_url}/authors?slug={quote_content['authorSlug']}")
        ...
        quote.set_thumbnail(url=f"{self.images_api_url}/200/{quote_content['authorSlug']}.jpg")
    ...
    async def tags(self, interaction: Interaction) -> None:
        ...
        tags_response = requests.get(self.quote_api_url + endpoint)
    ...
    async def on_error(self, interaction: Interaction, error: app_commands.AppCommandError) -> None:
        ...
        await interaction.response.send_message("Error while requesting data from the quote API!", ephemeral=True)
